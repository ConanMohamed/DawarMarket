...
def copy_order_data(request, order_id):
    """API endpoint to get order data for copying"""
    try:
        order = get_object_or_404(models.Order, id=order_id)
        items = order.items.all().select_related('product')
        
        # Format order text for copying
        order_text = f"""طلب رقم: {order.id}
العميل: {order.customer.full_name}
الهاتف: {order.customer.phone}
العنوان: {order.customer.address or 'غير محدد'}
العلامة المميزة: {order.customer.near_mark or 'غير محدد'}
وقت الطلب: {localtime(order.placed_at).strftime('%Y-%m-%d %H:%M')}
حالة الطلب: {order.order_status}

الأصناف:
"""
        
        for item in items:
            item_total = item.quantity * item.product.price_after_discount
            order_text += f"- {item.product.title} × {item.quantity} = {item_total:.2f} جنيه\n"
        
        order_text += f"\nالإجمالي: {order.total_price:.2f} جنيه"
...
@api_view(['GET'])
@permission_classes([IsAuthenticated, IsAdminUser])
def print_order_data(request, order_id):
    """API endpoint to get structured order data for printing"""
    try:
        order = get_object_or_404(models.Order, id=order_id)
        items = order.items.all().select_related('product')
        
        # Structure data for frontend printing
        order_data = {
            'order_info': {
                'id': order.id,
                'placed_at': localtime(order.placed_at).strftime('%Y-%m-%d %H:%M'),
            ...
            'customer_info': {
                ...
                'address': order.customer.address or 'غير محدد',
                'near_mark': order.customer.near_mark or 'غير محدد'
            },
            'items': [
                {
                    'product_title': item.product.title,
                    'quantity': item.quantity,
                    'unit_price': float(item.product.price_after_discount),
                    'total_price': float(item.quantity * item.product.price_after_discount)
                }
                for item in items
            ]
        }
...
class OrderActionsAPITest(TestCase):
    def test_copy_order_api(self):
        # Test copy endpoint
        url = reverse('copy-order-data', kwargs={'order_id': self.order.id})
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertTrue(response.data['success'])
        self.assertIn('order_text', response.data)
    
    def test_print_order_api(self):
        # Test print endpoint
        url = reverse('print-order-data', kwargs={'order_id': self.order.id})
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertTrue(response.data['success'])
